...
def eval(cfg: DictConfig):
    output_train_dir = get_latest_hydra_job_execution(["train_flood"])

    start_time = time.time()
    print_start_message(cfg, "inference")

    network_dir = os.path.join(output_train_dir, "flood")
    ...
    baseline_directory = env.baseline_directory()
    eval_network = ["python", os.path.join(baseline_directory, 'flood_eval.py')]
    eval_network_common_args = model_path + val_csv + model_name + gpu

    network_eval_preds_dir = os.path.join(cfg.output_dir, "flood_eval_preds")
    ...
    network_eval_fig_dir = os.path.join(cfg.output_dir, "flood_eval_fig")
    ...
    print_end_message(cfg, "inference", elapsed_time)


def print_start_message(cfg, action):
    print("==================================================================================")
    print(f"Flood network with model {cfg.network.model} {action} started...")
    print(f"Parameters used:")
    for key, value in cfg.network.items():
        print(f"\t{key}: {value}")


def print_end_message(cfg, action, elapsed_time):
    hours = int(elapsed_time // 3600)
    minutes = int((elapsed_time % 3600) // 60)
    seconds = int(elapsed_time % 60)

    print(f"Flood network with model {cfg.network.model} {action} finished. ")
    if hours > 0:
